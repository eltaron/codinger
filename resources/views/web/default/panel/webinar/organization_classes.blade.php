                        ...
                        <div class="col-12 col-lg-5">
                            <div class="form-group">
                                ...
                                <select name="type" class="custom-select">
                                    ...
                                    <option value="course" @if (request()->get('type') == 'course') selected @endif>
                                        {{ trans('product.course') }}</option>
                                    <option value="text_lesson" @if (request()->get('type') == 'text_lesson') selected @endif>
                            ...
                            <div class="form-group">
                                ...
                                <select name="sort" class="form-control">
                                    ...
                                    <option value="inexpensive"
                                        @if (request()->get('sort', null) == 'inexpensive') selected="selected" @endif>
                                        {{ trans('public.inexpensive') }}</option>
                                    <option value="bestsellers"
                                        @if (request()->get('sort', null) == 'bestsellers') selected="selected" @endif>
            ...
            <form action="" method="get">
                <div
                    ...
                    <label class="cursor-pointer mb-0 mr-10 text-gray font-14 font-weight-500"
                        for="freeClassesSwitch">{{ trans('panel.only_free_classes') }}</label>
                    <div class="custom-control custom-switch">
                        <input type="checkbox" name="free" @if (request()->get('free', '') == 'on') checked @endif
                                        ...
                                        @case(\App\Models\Webinar::$active)
                                            @if ($webinar->type == 'webinar')
                                                @if ($webinar->start_date > time())
                                                    <span
                                                        class="badge badge-primary !text-sm">{{ trans('panel.not_conducted') }}</span>
                                                @elseif($webinar->isProgressing())
                                                    <span
                                                        class="badge badge-secondary !text-sm">{{ trans('webinars.in_progress') }}</span>
                                                @else
                                                    <span
                                                        class="badge badge-secondary !text-sm">{{ trans('public.finished') }}</span>
                                                @endif
                                            @else
                                                <span
                                                    class="badge badge-secondary !text-sm">{{ trans('webinars.' . $webinar->type) }}</span>
                                            @endif
                                        @break

                                        @case(\App\Models\Webinar::$isDraft)
                                            <span class="badge badge-danger !text-sm">{{ trans('public.draft') }}</span>
                                        @break

                                        @case(\App\Models\Webinar::$pending)
                                            <span class="badge badge-warning !text-sm">{{ trans('public.waiting') }}</span>
                                        @break

                                        @case(\App\Models\Webinar::$inactive)
                                            <span class="badge badge-danger !text-sm">{{ trans('public.rejected') }}</span>
                                        @break
                                    @endswitch
                                </div>
                            ...
                            <div class="webinar-card-body !flex !flex-col !w-full !gap-2">
                                ...
                                <div class="!flex !items-center !justify-between">
                                    <a href="{{ $webinar->getUrl() }}" target="_blank">
                                        <h3 class="!text-2xl !font-bold !text-gray-800">
                                            {{ $webinar->title }}
                                            <span
                                                class="badge badge-dark !ml-2">{{ trans('webinars.' . $webinar->type) }}</span>
                                            @if ($webinar->private)
                                                <span
                                                    class="badge badge-danger !ml-2">{{ trans('webinars.private') }}</span>
                                            @endif
                                        </h3>
                                    </a>
                                </div>
                                ...
                                @if ($authUser->id != $webinar->teacher_id and $authUser->id != $webinar->creator_id)
                                    <div class="!flex !flex-col !text-base mt-3!">
                                        <span class="!text-gray-500">{{ trans('webinars.teacher_name') }}:</span>
                                        <span
                                            class="!font-semibold !text-gray-800">{{ $webinar->teacher->full_name }}</span>
                                    </div>
                                @elseif(
                                    ...
                                    <div class="!flex !flex-col !text-base mt-3!">
                                        <span class="!text-gray-500">{{ trans('webinars.organization_name') }}:</span>
                                        <span
                                            class="!font-semibold !text-gray-800">{{ $webinar->creator->full_name }}</span>
                                    </div>
                                @endif
                                <!-- Rating -->
                                ...
                                <div class="webinar-price-box !mt-1 !text-xl !font-semibold">
                                    @if ($webinar->price > 0)
                                        @if ($webinar->bestTicket() < $webinar->price)
                                            <span
                                                class="real !text-green-600">{{ handlePrice($webinar->bestTicket(), true, true, false, null, true) }}</span>
                                            <span
                                                class="off !ml-2 !line-through !text-gray-500">{{ handlePrice($webinar->price, true, true, false, null, true) }}</span>
                                        @else
                                            <span
                                                class="real !text-green-600">{{ handlePrice($webinar->price, true, true, false, null, true) }}</span>
                                        @endif
                                    @else
                                        <span class="real !text-green-600">{{ trans('public.free') }}</span>
                                    @endif
                                </div>
                                ...
                                <div class="!flex !w-full !justify-between !items-start !flex-wrap !gap-6 !mt-6">
                                    ...
                                    <div class="!flex !flex-col !text-lg ">
                                        <span class="!text-gray-500 !text-base">{{ trans('public.item_id') }}:</span>
                                        <span class="!font-bold !text-gray-800">{{ $webinar->id }}</span>
                                    </div>

                                    <div class="!flex !flex-col !text-lg ">
                                        <span class="!text-gray-500 !text-base">{{ trans('public.category') }}:</span>
                                        <span
                                            class="!font-bold !text-gray-800">{{ !empty($webinar->category_id) ? $webinar->category->title : '' }}</span>
                                    </div>

                                    @if ($webinar->isProgressing() and !empty($nextSession))
                                        <div class="!flex !flex-col !text-lg ">
                                            <span
                                                class="!text-gray-500 !text-base">{{ trans('webinars.next_session_duration') }}:</span>
                                            <span
                                                class="!font-bold !text-gray-800">{{ convertMinutesToHourAndMinute($nextSession->duration) }}
                                                Hrs</span>
                                        </div>
                                        ...
                                        @if ($webinar->isWebinar())
                                            <div class="!flex !flex-col !text-lg ">
                                                <span
                                                    class="!text-gray-500 !text-base">{{ trans('webinars.next_session_start_date') }}:</span>
                                                <span
                                                    class="!font-bold !text-gray-800">{{ dateTimeFormat($nextSession->date, 'j M Y') }}</span>
                                            </div>
                                        @endif
                                    @else
                                        <div class="!flex !flex-col !text-lg ">
                                            <span class="!text-gray-500 !text-base">{{ trans('public.duration') }}:</span>
                                            <span
                                                class="!font-bold !text-gray-800">{{ convertMinutesToHourAndMinute($webinar->duration) }}
                                                Hrs</span>
                                        </div>
                                        ...
                                        @if ($webinar->isWebinar())
                                            <div class="!flex !flex-col !text-lg ">
                                                <span
                                                    class="!text-gray-500 !text-base">{{ trans('public.start_date') }}:</span>
                                                <span
                                                    class="!font-bold !text-gray-800">{{ dateTimeFormat($webinar->start_date, 'j M Y') }}</span>
                                            </div>
                                        @endif
                                    @endif
                                    ...
                                    @if ($webinar->isTextCourse() or $webinar->isCourse())
                                        <div class="!flex !flex-col !text-lg ">
                                            <span class="!text-gray-500 !text-base">{{ trans('public.files') }}:</span>
                                            <span class="!font-bold !text-gray-800">{{ $webinar->files->count() }}</span>
                                        </div>
                                    @endif
                                    ...
                                    @if ($webinar->isTextCourse())
                                        <div class="!flex !flex-col !text-lg ">
                                            <span
                                                class="!text-gray-500 !text-base">{{ trans('webinars.text_lessons') }}:</span>
                                            <span
                                                class="!font-bold !text-gray-800">{{ $webinar->textLessons->count() }}</span>
                                        </div>
                                    @endif
                                    ...
                                    @if ($webinar->isCourse())
                                        <div class="!flex !flex-col !text-lg ">
                                            <span
                                                class="!text-gray-500 !text-base">{{ trans('home.downloadable') }}:</span>
                                            <span
                                                class="!font-bold !text-gray-800">{{ $webinar->downloadable ? trans('public.yes') : trans('public.no') }}</span>
                                        </div>
                                    @endif

                                    <div class="!flex !flex-col !text-lg ">
                                        <span class="!text-gray-500 !text-base">{{ trans('panel.sales') }}:</span>
                                        <span class="!font-bold !text-gray-800">{{ count($webinar->sales) }}
                                            ({{ (!empty($webinar->sales) and count($webinar->sales)) ? handlePrice($webinar->sales->sum('amount')) : 0 }})
                                        </span>
                                    </div>
                                </div>
