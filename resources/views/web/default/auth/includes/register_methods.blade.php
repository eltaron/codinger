...
@if($showOtherRegisterMethod)
    <div class="d-flex align-items-center wizard-custom-radio mb-20">
        <div class="wizard-custom-radio-item flex-grow-1">
            <input type="radio" name="type" value="email" id="emailType" class="" style="background:var(--primary);" {{ (($registerMethod == 'email' and empty(old('type'))) or old('type') == "email") ? 'checked' : '' }}>
            <label class="font-12 cursor-pointer px-15 py-10" for="emailType">{{ trans('public.email') }}</label>
        </div>

        <div class="wizard-custom-radio-item flex-grow-1">
            <input type="radio" name="type" value="mobile" id="mobileType" class="" style="background:var(--primary);" {{ (($registerMethod == 'mobile' and empty(old('type'))) or old('type') == "mobile") ? 'checked' : '' }}>
