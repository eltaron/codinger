    ...
    */
    ...
    protected function attemptLogin(Request $request)
    {
        ...
        if ($user) {
            ...
            if ($passwordCheck) {
                $data = \Illuminate\Support\Facades\Auth::login($user);
                return true;
            }
        }
        return false;
   ...
   public function afterLogged(Request $request, $verify = false)
    {
        ...
        // ------------------ BEGIN MAC ADDRESS LOGIC ------------------

        // 1. حاول استخلاص الماك ادرس الجديد
        $macAddress = $this->getMacAddress($request->header('User-Agent'));
        Log::info('Login attempt', ['user_id' => $user->id, 'retrieved_mac' => $macAddress]);

        // 2. تحقق إذا كان المستخدم لديه ماك ادرس مسجل ومُعرّف بالفعل
        if (!empty($user->mac_address)) {

            // لديه ماك مسجل، فلنقارن
            if ($user->mac_address !== $macAddress) {

                // الماك ادرس لا يتطابق - قم بتسجيل الخروج
                Log::warning('MAC Address Mismatch - Logging out user.', [
                    'user_id' => $user->id,
            ...
            // 3. الماك ادرس في قاعدة البيانات فارغ، ولكن تمكنا من الحصول على واحد جديد من الطلب
            // هذا هو أول تسجيل دخول من جهاز مُعرّف. قم بتسجيله

            Log::info('First-time MAC address registration for user.', [
                'user_id' => $user->id,
                'new_mac' => $macAddress
            ]);
        ...
        // 4. إذا كان الماك ادرس في قاعدة البيانات فارغًا والجديد أيضًا فارغ (null)، لا تفعل شيئًا واستمر

        // ------------------- END MAC ADDRESS LOGIC -------------------
